...
```{r, echo=FALSE, fig.cap= "Drug Consumption by Gender (left) and Age (right).", fig.align="center", fig.align="center", fig.width=6, fig.height=2}
...
```{r, echo=FALSE, fig.cap= "Country Distribution (left) and Ethnicity Distribution (right) Across Drug Consumption Categories.", fig.align="center", fig.align="center", fig.width=6, fig.height=2}
...
```
From the Country and Ethnicity graphs, the source of the data is evident. The majority of the data is collected from individuals who are white or of mixed white ancestry, and reside in countries with a significant white population.
...
Neural networks are inspired by the architecture of biological neural systems, where processing elements, analogous to neurons, form the core of the network. Each neuron receives one or more inputs, processes those inputs, and produces an output.
The main components of neural network information processing include inputs, weights, summation functions.

With 5 hidden layers, the neural model's balanced accuracy for training data and validation data are 0.29 and 0.30, showing that there is no overfitting. Finally, the balanced accuracy for test data is 0.347 and combined with confusion matrix, we can gain some summary.
