...
```{r, echo=FALSE, fig.cap= "Country Distribution (left) and Ethnicity Distribution (right) Across Drug Consumption Categories.", fig.align="center", fig.align="center", fig.width=6, fig.height=2,include=FALSE}
...
grid.arrange(g5, g6, ncol = 2, widths = c(1, 2.25))
...
\
Classification trees are a non-parametric method well-suited for complex data sets.
They start at the root node, assessing features to classify instances and branching out to child nodes, each representing different features.
This process recursively continues until reaching a leaf node where the instance is classified.
However,when a tree grows large with tons of splits, we may faced the circumstance of overfitting.
In order to avoid this situation as much as possible and reduce the impact of overfitting on the generalization of the model, we can prune a full tree to a subtree by using cost-complexity pruning.

In this question, due to the imbalance of data between categories, we prune the tree based on the value of balanced accuracy.
...
-   Construct the full tree with $minsplit = 2$ ,$maxdepth = 30$, $minbucket = 7$ .

-   Set $tolerance=0.06$ to test whether the model is overfitting ($|test_{balancedacu}-valid_{balanceacu}|<=tolerance$)

-   Find out the best cp of tree with maximum balanced accuracy on validation set

-   Pruned the tree with best cp and calculated the balanced accuracy of pruned tree on testing set.
...
According to the confusion matrix, the numbers on the diagonal being greater than the sum of the rest suggests that this may be a prediction with good accuracy.
Meanwhile, it is obvious that 180 observations in category "Never used" is successfully predicted, whilst for other categories the amount of "True Positive" is quite lower.
This situation may be due to an imbalance in the number of data points across different categories.
It's clear that the 'Never Used' category has far more data points than the other categories, hence we should consider the balanced accuracy of the classification tree model.
Here is the table of balanced accuracy for testing data set under the pruned tree:
...
The table above indicates that the balanced accuracy for the categories "Used in Last Decade" and "Used Over a Decade Ago" is nearly identical, averaging around $0.566$.
There is a slight improvement of approximately $0.01$ for the "Never Used" category.
However, the category "Used in Last Year to Day" exhibits the lowest balanced accuracy.
The overall mean balanced accuracy of the model stands at $0.561149$, which esceeds the expected accuracy of random classification.
...
Based on the analysis above, In K-nearest Neighbours,Linear Discriminant Analysis,Support Vector Machines,Neural Network,and Tree-based methods, the classification tree has the highest balanced accuracy.
Therefore, we choose it for classifying and predicting medication timing.
