...
# Explanatory Data Analysis
  ...
  We checked the structure of the data and found that it's a good multi-categorical dataset without any missing values.And both the response variable and explanatory variables are categorical.
  We can't use normal techniques applied to continuous variables.
To effectively analyze and model the relationships between the response variable and explanatory variables, we need to aalysis the clear distribution of the different varibales.
And using bar charts also be the good method.
...
```{r, echo=FALSE, fig.cap="Distribution of Drug Use Categories (left) and Drug Consumption by Age (right).", fig.align="center", fig.align="center", fig.width=6, fig.height=2}
theme1 <- theme(
...
grid.arrange(g1, g2, ncol = 2, widths = c(1, 2.5))
```

  To number of drug users against different Drug Usage Categories and ages graphs,the 'never used' group constitutes the majority of drug users, and the number of drug users declines with increasing age.


```{r, echo=FALSE, fig.cap= "Drug Consumption by Gender (left) and Edu (right).", fig.align="center", fig.align="center", fig.width=6, fig.height=2,include=FALSE}
...
```{r, echo=FALSE, fig.cap= "Gende Edu Country and Ethnicity Distribution Across Drug Consumption Categories.", fig.align="center", fig.align="center", fig.width=7, fig.height=4}
...
grid.arrange(g3,g4,g5, g6, ncol = 2, widths = c(1, 2.25))


```

From the Country and Ethnicity graphs, the source of the data is evident.
The majority of the data is collected from individuals who are white or of mixed white ancestry, and reside in countries with a significant white population.
...
```{r echo=FALSE, fig.cap= "Drug last used time v.s. Personality Measures (Nscore, Escore, Oscore, Ascore).", fig.align="center",fig.width=6, fig.height=4}
...
It's clear that the bar charts of nscore, escore, oscore, cscore, and ascore have similar structures, with normal distributions across different drug usage times,which indicates these scores won't influence drug usage.

```{r, echo=FALSE, fig.cap= "Drug last used time v.s. Personality Measures(Cscore, Impulsive, SS).", fig.align="center",fig.width=6, fig.height=4}
...
## The K-Nearest Neighbors (KNN)
  ...
  K is the important parameter in KNN, it decide how many closest points will be used to determine the class of the new point.,We should find a suitable value to fit the model.
  In this project, we use the Euclidean distance, "for" iteration to find best parameter in this project, and the criterion is the balanced_accuracy
  For this data set, we will iterate k from 1 to 30 ,and find the best k with the highest value.When we get the best K, we will use confusion matrix to see its balanced accuracy, and compare it with other models.
...
print(mean_value)
```
    From the graph,the classification model achieves the best performance when k = 1. And use k =1 to fit the test dataset, the balanced accuracy is 0.5357226.
...
kable(result_table, caption = "Balanced Accuracy Values for LDA and QDA Models.") %>%
  kable_styling(latex_options = c("scale_down"))
```

From the balanced accuracy tale for LDA and QDA models, the LDA model performs better in consistency than the QDA.
Despite of the QDA model offers more flexible boundaries (as QDA achieves higher balanced accuracy values across all classes), there is a significant drop from the training to validation and test sets in balanced accuracy in both the LDA and QDA model, underscoring potential overfitting and less robust predictive ability for both models.
This suggests that neither model is well-fitted to the drug consumption dataset relevant to the study.
...
kable(svm_data_wide, 
      caption = "Balanced Accuracy for SVM Models.", 
      booktabs = TRUE) %>%
  kable_styling(latex_options = c("scale_down")) 
```

The summary table of balanced accuracy scores for the SVM models with various kernels demonstrates that: the performance of the linear model and Polynomial Kernel with Degree 2 model both appear to be consistent across datasets, indicating stable performance; the Polynomial Kernel with Degree 3 model showed higher balanced accuracy rates on the training set compared to the other kernels.However, some overfitting issues may be present; the RBF kernel shows reletively consistent performance across datasets; the sigmoid kernel shows lower balanced accuracy trends than the other kernels across datasets, indicating that it may not be suitable for this classification project. Therefore the RBF kernel offers slightly outstanding adaptability across training, validation and testing data distributions. However, none of the kernels is performing strong performance, with the balanced accuracy values around 0.5 across all classes with all kernels as summarized. This result may indicate that the drug consumption by time does not exhibit strong linear or simple non-linear separability, model tuning should be considered for next step by hyperparameter optimization, or that SVM may not be the optimal classifier choice for solving this classification problem.
...
The main components of neural network information processing include inputs, weights, summation functions.

With 5 hidden layers, the neural model's balanced accuracy for training data and validation data are 0.29 and 0.30, showing that there is no overfitting.
Finally, the balanced accuracy for test data is 0.347 and combined with confusion matrix, we can gain some summary.
...
conf_matrix_plot <- matrix(c(867, 92, 88, 71,
                             0, 0, 0, 2,
                             26, 52, 16, 7,
                            14, 7, 48, 23),
                      nrow = 4, byrow = TRUE)

colnames(conf_matrix_plot) <- c("Never", "Decade+", "LastDec", "LastYear")
rownames(conf_matrix_plot) <- c("Never", "Decade+", "LastDec", "LastYear")
...
-   There is a large concentration of instances in the "Never Used" category, showing a skewed distribution of the dataset towards this category, which indicates that this model might struggle to predict less when model predict categories such as 'Used in Last Decade'.
...
Classification trees are a non-parametric method well-suited for complex data sets. They start at the root node, assessing features to classify instances and branching out to child nodes, each representing different features. This process recursively continues until reaching a leaf node where the instance is classified.However,when a tree grows large with tons of splits, we may faced the circumstance of overfitting. In order to avoid this situation as much as possible and reduce the impact of overfitting on the generalization of the model, we can prune a full tree to a subtree by using cost-complexity pruning.

In this question, due to the unbalance of data between categories, we prune the tree based on the value of balanced accuracy.
To be more specific, We consider finding the cp of the tree that maximizes balanced accuracy on validation set, while ensuring that the difference between the training set and the validation set is less than the specified tolerance.

-   Construct full tree on training data with $minsplit = 2$ ,$maxdepth = 30$, $minbucket = 7$.

-   Condition:Set $tolerance=0.06$ test overfitting ($|test_{balance}-valid_{balance}|<=tolerance$)

-   Find out the best cp of tree with maximum balanced accuracy on validation set

-   Pruned the tree with best cp and calculated balanced accuracy on testing set.
...
```{r echo=FALSE}
# definite the function to prune
find_best_cp <- function(full_tree, training_data, validation_data, cp_start, cp_end, steps, tolerance) {
  ...
  for(cp in cp_values) {
    ...
    #predict the result of train data in pruned tree and constrcut confusion matrix
    predictions_train <- predict(pruned_tree, training_data, type = "class")
    conf_matrix_train <- confusionMatrix(predictions_train, 
                                         training_data$Amyl_grouped)
    
    #calculate accuracy balanced accuracy and mean balanced accuracy for training data
    balance_train <- conf_matrix_train$byClass[,"Balanced Accuracy"]
    mean_balance_train<-mean(balance_train)
    ...
    conf_matrix_validation <- confusionMatrix(predictions_validation,
                                              validation_data$Amyl_grouped)
    
    accuracy_validation<-conf_matrix_validation$overall["Accuracy"]
    balance_valid <- conf_matrix_validation$byClass[,"Balanced Accuracy"]
    mean_balance_valid<-mean(balance_valid)
    # examin whether the balanced accuracy between train and valid is smaller than tolerance to avoid overfitting
    if((abs(mean_balance_train-mean_balance_valid)) <= tolerance){
      if(mean_balance_valid > mean_balanced_accuracy){
         mean_balanced_accuracy<- mean_balance_valid
         best_cp <- cp
         best_tree <- pruned_tree
         best_accuracy<-accuracy_validation
         balance_accuracy<-balance_valid
         confusion_matrix<-conf_matrix_validation
      }
    }
  }
  
  list(best_tree = best_tree, 
       best_cp = best_cp, 
...
set.seed(12345)
full_tree <- rpart(Amyl_grouped~., data=train_data, method="class", control=rpart.control(minsplit = 2 ,maxdepth = 30, cp = -1, minbucket = 7))
printcp(full_tree)
```
...
```{r echo=FALSE, results='hide'}
# balanced accuracy of test data table for best accuracy tree 
predictions_test <- predict(best_tree, test_data, type = "class")
      
# calculate the weighted_balanced_accuracy_test 
conf_matrix_test <- confusionMatrix(predictions_test,test_data$Amyl_grouped)
balance_test <- conf_matrix_test$byClass[,"Balanced Accuracy"]
accuracy_test<- conf_matrix_test$overall["Accuracy"]
mean_balance_test<-mean(balance_test)
print(balance_test)
print(mean_balance_test)
```
...
Based on the process above, the cp of the pruned tree is $0.00615578$.
Here is the confusion matrix of testing data set under the pruned tree:
...
confusion_matrix <- matrix(c(180, 22, 30, 12, 8, 5, 3, 0, 10, 2, 8, 2, 0, 0, 0, 1), 
                           nrow = 4, byrow = TRUE,
                           dimnames = list(c("Never", "Decade+", "LastDec", "LastYear"),
                                           c("Never", "Decade+", "LastDec", "LastYear")))

confusion_data_frame <- data.frame(confusion_matrix)
knitr::kable(confusion_data_frame,caption = "Confusion matrix for testing data")%>%
  kable_styling(latex_options = c("scale_down"))
```

The confusion matrix suggests good predictive accuracy, with a notably high number of correct predictions in the 'Never Used' category compared to others.
The much lower 'True Positive' rates in other categories might reflect insufficient data.
Given that 'Never Used' has more data points, evaluating the model's balanced accuracy is essential.
Below is the balanced accuracy table for the pruned tree model on the test dataset:

```{r echo=FALSE,warning=FALSE, fig.align="center"}
#| label: tbl-tree1
#| tbl-cap: Balanced accuracy for different class in testing data
# build up dataframe for result previous
tree_test <- data.frame(
  Class = c("Never Used", "Used Over a Decade Ago", "Used in Last Decade", "Used in Last Year to Day", "Average Balanced Accuracy"),
  Accuracy = c(0.5780749, 0.5645534, 0.5686354, 0.5333333, 0.5611492)
)
# construct a table
knitr::kable(tree_test, col.names = c("Class", "Balanced Accuracy"), caption = "Balanced accuracy of testing data")%>%
  kable_styling(latex_options = c("scale_down"))
```

The table shows that the balanced accuracy for "Used in Last Decade" and "Used Over a Decade Ago" is similar at around 0.566. The "Never Used" category performs slightly better with an increase of 0.01 in accuracy, while "Used in Last Year to Day" has the lowest accuracy. Overall, the model's mean balanced accuracy is 0.561149, exceed random chance.

# Model Comparison


Given that our response variable is categorized into multiple classes and there is an imbalance in our dataset with the 'Never Used' category significantly outnumbering others.  we chose balanced accuracy for model comparisons.
...
knitr::kable(amyl_usage, caption = "Balanced accuracy for different models")
```

Below is the specific decision tree diagram.

```{r echo= FALSE,fig.width=10, fig.height=6}
#| label: fig-treebest
#| fig-cap: figure of the best tree.

# plot the best tree on training data.
rpart.plot(best_tree,type=2,extra=4)
```

Except for Escore and Ethnicity, the remaining attributes participate in the decision-making of the classification tree, and SS, Country, Gender, and Age are more sensitive in the classification decision than other attributes in the tree.


# Conclusion

To summarize, the analysis demonstrates that the classification tree method is relatively effective for predicting the timing of an individual's last drug intake time. Starting with broad categorizations based on Age, Country, Gender, and ImpSSscales  offers an initial layer of prediction. For enhanced precision, further classification is recommended by incorporating detailed factors such as Ascore, Education, and Oscore. This approach ensures a more accurate prediction of the last medication usage.
