...
For constructing the neural network model, we use '**Amyl_grouped**' as the response variable and other variables as predicted variables. And with hidden layer1, 2, 3, 4 or 5, we find model 5 hidden layers has appropriate explain even though model with 2 layers has the highest accuracy. Since model with 2 hidden layers have failed to learn enough information during training to make effective predictions about the class. This model's balanced accuracy for training data and validation data are 0.69 and 0.70, showing that there is no overfitting in neural model. Finally, the balanced accuracy for test data is 0.58 and combined with confusion matrix, we can figure some summary.
