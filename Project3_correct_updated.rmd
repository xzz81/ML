...
For constructing the neural network model, we use '**Amyl_grouped**' as the response variable and other variables as predicted variables. And with hidden layer1, 3or 5, we find model with 5 hidden layers has the highest accuracy. This model's balanced accuracy for training data and validation data are 0.29 and 0.30, showing that there is no overfitting in neural model. Finally, the balanced accuracy for test data is 0.34 and combined with confusion matrix, we can figure some summary.
