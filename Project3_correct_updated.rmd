...
To apply support vector machines technique to predict the grouped categories of Amyl nitrite usag, various kernels (polynomial with degrees 3, radial basis function, and sigmoid respectively) are employed to train 5 distinct SVM models.
...
```
This result may indicate that the drug consumtion by time does not exhibit strong linear or simple non-linear separability, or that SVM may not be the optimal classifier choice for solving this classification problem without further feature engineering or consideration of other more complex models.
